...
- `business-workspace/`: overview tab, profile/general information editing, selected parent services, quick sub-service add/remove, job/employee summaries, delete/leave business. The hero action area includes a `View Public Page` button for active business members, which navigates to the customer-facing `/:businessCode` page using the current workspace `businessCode`.
