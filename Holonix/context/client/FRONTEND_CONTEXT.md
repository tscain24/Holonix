...
- `public-business.service.ts`: public business profile API methods, including public daily availability lookups used by the booking flow and the authenticated Stripe setup checkout-session creation call used by the public cart.
...
- The create-business screen now uses a single location flow instead of a full-address/general-location toggle: `Address 1` is optional, `City` and `State` are required, the street-address field offers address suggestions, and the city field offers city/state suggestions. When there is no street address, the form still needs confirmed coordinates before continuing, but it can now derive them either from a picked city/state suggestion or by resolving the typed city/state/ZIP during the continue/launch flow. The step-2 business-model chooser is currently hidden and the form defaults new businesses to service-based behind the scenes.
...
- The dev command is `npm start`, but the repo-level `.\start-dev.ps1` is preferred because it coordinates backend and frontend ports. The Angular dev server runs with SSL on `https://localhost:4200` so Stripe secure-origin features such as wallets and autofill can be tested locally, and the startup script exports the current localhost development certificate into `.dev-runtime/certs/` so Angular and ASP.NET share the same local certificate chain.
